...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('sendMessage', async (data) => {
      const { prompt, chatId = uuidv4() } = data;
      console.log('User asked Manee:', prompt);

      try {
        await pool.query('INSERT IGNORE INTO chats (chat_id, title) VALUES (?, ?)', [chatId, prompt.substring(0, 50)]);
        ...
        await pool.query('INSERT INTO messages (chat_id, role, content) VALUES (?, ?, ?)', [chatId, 'manee', fullManeeResponse]);
  ...
  httpServer.listen(port, () => {
    console.log(`> Manee AI is live and saving to TiDB Cloud`);
  });
});